...
@section('content')
    <div class="container mx-auto mt-8">
        <div class="max-w-md mx-auto bg-white rounded p-6 shadow-md">
            <h2 class="text-2xl font-semibold mb-4">{{ __('Create Tour') }}</h2>
            ...
            <form method="POST" action="{{ route('admin.create_tour') }}">
                ...
                <div class="mb-4">
                    ...
                    <select name="user_id" id="user_id" class="mt-1 p-2 w-full border rounded-md">
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}">{{ $user->name }} - {{ $user->organisation ? $user->organisation->name : 'No Organisation' }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-md">{{ __('Create Tour') }}</button>
                </div>
            </form>
        </div>
    </div>
